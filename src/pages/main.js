...
export default function Main() {
    ...
    const getUsers = async () => {
        await axios.get("https://reqres.in/api/users?page=1&per_page=4")
            .then((response) => {
    ...
    const Page = async (pageNumber) => {
        await axios.get("https://reqres.in/api/users?page=" + pageNumber + "&per_page=4")
            .then((response) => {
                setData(response.data.data);
            }).catch((err) => {
                if (err.response) {
        ...
        <>
                                ...
                                {data.map(user => (
                                    <TableRow key={user.id}>
                                        <TableCell collapsing>
                                            <ButtonGroup>
                                                <Button onClick={() => getUser(user.id)}><Icon name='edit' /></Button>
                                                <ButtonOr />
                                                <Button onClick={() => openModalDelete(user.id, user.avatar, user.first_name, user.last_name)}><Icon name='trash alternate outline' /></Button>
                                            </ButtonGroup>
                                        </TableCell>
            ...
            >
                <Header icon>
                    ...
                    {selectedUser && (
                        <div>
                            <Image src={selectedUser.avatar} avatar /> {selectedUser.firstName} {selectedUser.lastName}
                        </div>
                    )}
                </Header>
                <ModalContent>
                ...
                <ModalActions>
                    ...
                    <Button color='green' inverted onClick={() => deleteRow(selectedUser.id)}>
                ...
                <ModalContent image>
                    ...
                    <ModalDescription>
                        ...
                        <p>
                            {user && (
                                <div>
                                    <Input onChange={e => setFirstName(e.target.value)} iconPosition='left' placeholder='Nome'>
                                        <Icon name='user outline' />
                                        <input />
                                    </Input>
                                    <Input onChange={e => setLastName(e.target.value)} iconPosition='left' placeholder='Sobrenome'>
                                        <Icon name='user outline' />
                                        <input />
                                    </Input>
                                    <Input onChange={e => setEmail(e.target.value)} iconPosition='left' placeholder='Email'>
                                        <Icon name='at' />
                                        <input />
                                    </Input>
                                </div>
